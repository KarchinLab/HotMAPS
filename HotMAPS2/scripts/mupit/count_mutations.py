    ...
    for filename in os.listdir(opts['data_dir']):
        if filename[:6] == 'mupit.':
            ...
            with open(out_filepath, 'w') as wf:
                for pdb in pdbs:
                    seqress = list(seqres_by_pdb[pdb].keys())
                    seqress.sort()
                    line_info = [pdb, str(len(list(no_sample[pdb].keys()))),
