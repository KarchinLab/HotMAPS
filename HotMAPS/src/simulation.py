...
def generate_null_dist(struct_id, model_info, chain_info,
    ...
    # sim_null_dist = stats.itemfreq(sim_density) itemfreq is deprecated

    values, counts = np.unique(sim_density, return_counts=True)
    sim_null_dist = np.column_stack((values, counts))
