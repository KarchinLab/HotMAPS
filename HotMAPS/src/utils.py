...
def read_pdb_info(pdb_info_path):
...
def read_af_structure(pdb_path, prot_name, quiet=True):
    """Reads in a PDB structure predicted by alphafold.

    Will read uncompressed PDB structures from alphafold output.
    ...
    # skip if there is no pdb for it
    if not pdb_path:
        logger.debug('Skipping unavailable pdb {0}'.format(prot_name))
        return None
    ...
    try:
        ...
        if pdb_path.endswith('.gz'):
            with gzip.open(pdb_path, 'rb') as handle:
                structure = pdb_parser.get_structure(prot_name, handle)
        else:
            structure = pdb_parser.get_structure(prot_name, pdb_path)
        ...
        logger.info('Failed reading in structure {0}'.format(prot_name))
        return None
